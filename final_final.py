...
def run_probe(embeddings, labels, stack_idx, device, seed=42):
    ...
    # Singular value analysis: fraction of squared spectral norm captured by top SV
    sv_squared  = S ** 2
    top_sv_frac = (sv_squared[0] / sv_squared.sum()).item()
    ...
    print(f"    Top SV fraction      = {top_sv_frac*100:.1f}%  "
          f"(singular values: {[f'{s:.3f}' for s in S.tolist()]})")

    return task_acc, ctrl_acc, w, top_sv_frac
...
def run(args):
    ...
    for stack_idx in range(args.k):
        ...
        ask_acc, ctrl_acc, w, top_sv_frac = run_probe(
        ...
        all_probe_results[f"stack_{stack_idx}"] = {
            ...
            "top_sv_fraction":      round(top_sv_frac * 100, 2),
        }
